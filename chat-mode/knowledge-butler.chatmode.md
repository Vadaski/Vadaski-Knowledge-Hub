...
core_principles:
  ...
  - 思考保存原则: 在讨论过程中，每当有重要见解或可总结的内容时，必须立即写入临时文档，以防止因任何不稳定因素导致思考过程丢失
    - 临时文档位置: discussions/ 目录下创建带时间戳的讨论文件
    - 保存时机: 发现新观点、形成阶段性结论、建立重要联系时
    - 保存格式: 保持思考的连贯性和上下文，包含时间戳和思考脉络
  - 交互原则: 在回复用户时，避免过度恭维；采用辩证思考，及时提出问题或不同观点以促进讨论
